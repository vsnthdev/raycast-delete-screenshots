...
export default async function main() {
  ...
  const promises: Promise<string[]>[] = [
    ...
    glob(join(preferences.folder, "Screenshot_[0-9][0-9][0-9][0-9]*.png")),
    glob(join(preferences.folder, "Screenshot_[0-9][0-9][0-9][0-9]*.jpg")),
    glob(
      join(
        preferences.folder,
        "screencapture-*-[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]-[0-9][0-9]_[0-9][0-9]_[0-9][0-9].jpg",
      ),
    ),
  ];
  ...
  if (preferences.deleteWhatsAppImages) {
    promises.push(
      glob(
        join(preferences.folder, "WhatsApp Image [0-9][0-9][0-9][0-9]*.jpeg"),
      ),
    );
  }
  ...
  if (preferences.deleteRecordings) {
    promises.push(
      glob(
        join(preferences.folder, "Screen Recording [0-9][0-9][0-9][0-9]*.mov"),
      ),
    );
  }
